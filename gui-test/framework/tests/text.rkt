...
(define (text:ports-tests)
  ...
  (let ()
    (define t (new (text:ports-mixin text:wide-snip%)))
    (define op (send t get-out-port))
    (write-special (horizontal "a" "b" "c") op)
    (flush-output op)
  ...
  (let ()
    (define t (new (text:ports-mixin text:wide-snip%)))
    (define op (send t get-out-port))
    (write-special (vertical "a" "b" "c") op)
    (flush-output op)
    (check-equal? (send t get-text) "a\nb\nc"))
  ...
  (let ()
    (define t (new (text:ports-mixin text:wide-snip%)))
    (define op (send t get-out-port))
    (write-special (framed-markup (horizontal "a" "b" "c")) op)
    (flush-output op)
  ...
  (let ()
    ...
    (write-special (horizontal "a" (image-markup bmp "x") "b") op)
    (flush-output op)
  ...
  (let ()
    (define t (new (text:ports-mixin text:wide-snip%)))
    (define op (send t get-out-port))
    (write-special (horizontal
                    (number 1/3 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'decimal)
                    " "
                    (number 4/3 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'mixed)
                    " "
                    (number 4/3 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'improper)
                    " "
                    (number #i0.5 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'decimal)
                    " "
                    (number #e0.5 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'decimal))
                   op)
    (flush-output op)
  ...
  (let ()
    ...
    (write-special (horizontal
                    ...
                    (number 4/3 #:exact-prefix 'never #:inexact-prefix 'never #:fraction-view 'improper)
                    " "
                    (number #i0.5 #:exact-prefix 'never #:inexact-prefix 'always #:fraction-view 'decimal)
                    " "
                    (number #e0.5 #:exact-prefix 'always #:inexact-prefix 'never #:fraction-view 'decimal))
                   op)
    (flush-output op)
  ...
  (let ()
    (define t (new (text:ports-mixin text:wide-snip%)))
    (send t insert "one\n")
    (send t do-submission)
    (send t insert " two\n")
    (send t do-submission)
    (define ip (send t get-in-port))
    (check-equal? #\o (read-char ip))
    (check-equal? '(1 1 2) (call-with-values (lambda () (port-next-location ip)) list))
    (check-equal? #\n (read-char ip))
    (check-equal? #\e (read-char ip))
    (check-equal? '(1 3 4) (call-with-values (lambda () (port-next-location ip)) list))
    (check-equal? #\newline (read-char ip))
    (check-equal? '(2 0 5) (call-with-values (lambda () (port-next-location ip)) list))
    (check-equal? #\space (read-char ip))
    (check-equal? '(2 1 6) (call-with-values (lambda () (port-next-location ip)) list))
    (check-equal? #\t (read-char ip))
    (check-equal? #\w (read-char ip))
    (check-equal? #\o (read-char ip))
    (check-equal? '(2 4 9) (call-with-values (lambda () (port-next-location ip)) list))
    (check-equal? #\newline (read-char ip))
    (check-equal? '(3 0 10) (call-with-values (lambda () (port-next-location ip)) list)))
